...
from models import create_tables, Shop, Stock, Book, Sale, Publisher

DSN = r"postgresql://postgres:603306MuS@localhost:5432/bookhouse"
...
def get_shops():
    """ Выполняется поиск магазина, в котором есть книги конкретного издателя. """
    ...
    if str.isdigit(''):
        if command == "id":
            ...
            _a = search_data.filter(Publisher.id == publisher_id).all()
    else:
        if command == "name":
            publisher_name = input("Please, enter the publisher`s name: ")
            _b = search_data.filter(Publisher.name == publisher_name).all()
